    ...
    internal static class LanguageHelper
    {
        public static bool IsNonEnglish(string? text)
        {
            ...
            foreach (char c in text)
            {
                ...
                totalLetters++;
                var codePoint = char.ConvertToUtf32(text, text.IndexOf(c));
        ...
        private static bool IsCyrillic(int cp) => cp >= 0x0400 && cp <= 0x04FF;
        private static bool IsArabic(int cp) => cp >= 0x0600 && cp <= 0x06FF;
        private static bool IsGreek(int cp) => cp >= 0x0370 && cp <= 0x03FF;
        private static bool IsThai(int cp) => cp >= 0x0E00 && cp <= 0x0E7F;
        private static bool IsHebrew(int cp) => cp >= 0x0590 && cp <= 0x05FF;
    }
}