...
namespace Scrapers.Services
{
    public class PubMedScraperService
    {
        ...
        {
            var investigators = await _context.Investigators.ToListAsync(cancellationToken).ConfigureAwait(false);
            int totalPapers = 0;
            ...
            foreach (var investigator in investigators)
            {
                ...
                if (!string.IsNullOrEmpty(investigators[0].NcbiId))
                {
                    investigator.NcbiId = investigators[0].NcbiId;
                }
                if (!string.IsNullOrEmpty(investigators[0].OrcidId))
                {
                    investigator.OrcidId = investigators[0].OrcidId;
                }
