...
### Migration Startup Order — Schema Before Seed
- **Rule:** Any code that queries the database at application startup must run AFTER `MigrateAsync()`.
  Never call seed methods, data lookups, or entity queries before the schema is created.
- **Why (PR #291):** `SeedMeshDescriptors()` was called before `MigrateSchemaAsync()` in `DataApi/Program.cs`.
  It queried `context.MeshDescriptors.AnyAsync()`, but the `mesh_descriptors` table didn't exist yet,
  crashing DataApi with `42P01: relation "mesh_descriptors" does not exist`.
- **Prevention:** Always review `DataApi/Program.cs` startup order when adding new `DbSet` access or
  seed logic during initialization. Schema must be the first database interaction.
