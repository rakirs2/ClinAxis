...
- Database integration tests live under `Scrapers.IntegrationTests` and must connect to the configured PostgreSQL instance (see `POSTGRES_CONNECTION_STRING`). For local IDE runs you may place the connection string in `Scrapers.IntegrationTests/.integrationtests.env`. GitHub Actions uses the postgres service container defined in `.github/workflows/dotnet.yml` and automatically runs both unit and integration suites.
